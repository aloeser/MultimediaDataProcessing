...
class HuffmanNode {
    ...
    void insert_length_symbol_pairs_recursively(const uint8_t depth, std::vector<std::pair<uint8_t, uint8_t>>& length_symbol_pairs ) {
        if (_left) {
            // a node has two children, or none, never just one
            _left->insert_length_symbol_pairs_recursively(depth + 1, length_symbol_pairs);
            _right->insert_length_symbol_pairs_recursively(depth + 1, length_symbol_pairs);
        } else {
            // node has no children -> leaf -> insert code into huffman table
            const auto pair = std::make_pair(static_cast<uint8_t>(depth), _byte_value);
            length_symbol_pairs.push_back(pair);
        }
    }
...
public:
    ...
    HuffmanTable get_huffman_table() {
        std::vector<std::pair<uint8_t, uint8_t>> length_symbol_pairs;
        insert_length_symbol_pairs_recursively(0, length_symbol_pairs);
        std::sort(length_symbol_pairs.begin(), length_symbol_pairs.end()); // sort vector by length, then by symbol

        HuffmanTable huffman_table;
        uint8_t current_length = 0;
        uint32_t current_code = 0;
        for (const auto [length, symbol] : length_symbol_pairs) {
            while (current_length < length) {
                current_length++;
                current_code = (current_code << 1);
            }

            const std::string huffman_code = uint32_to_binary_string(current_code, current_length);
            huffman_table[symbol] = huffman_code;
            std::cout << "Byte " << static_cast<size_t>(symbol) << " has a code of length " << static_cast<size_t>(current_length) << ": " << huffman_code << std::endl;
            current_code++;
        }
...
void write_header(std::ostream& os, const HuffmanTable& huffman_table, bitwriter& bw) {
    ...
    uint8_t num_entries;
    if (huffman_table.size() == 256) {
        num_entries = 0;
    } else {
        num_entries = huffman_table.size();
    }
    raw_write(os, num_entries);

    // we need to write the entries sorted by length and then byte value
    std::vector<std::pair<uint8_t, uint8_t>> length_symbol_pairs;
    for (const auto& [symbol, huffman_code] : huffman_table) {
        std::cout << "encoding symbol " << static_cast<size_t>(symbol) << " with " << huffman_code << std::endl;
        length_symbol_pairs.push_back(std::make_pair(static_cast<uint8_t>(huffman_code.length()), symbol));
    }
    std::sort(length_symbol_pairs.begin(), length_symbol_pairs.end());


    for (const auto [length, symbol] : length_symbol_pairs) {
        std::cout << "encoding symbol " << static_cast<size_t>(symbol) << " with a code of length " << static_cast<size_t>(length)  << std::endl;
        bw.write(symbol, 8);
        bw.write(length, 5);
        // bw.write(byte_code, length); // not required if using canonical huffman codes
    }
}
...
void decode_huffman(std::istream& is, std::ostream& os) {
    const uint64_t huffman2 = 0x485546464d414e32;
    uint64_t format;
    raw_read(is, format);
    ...
    for (size_t symbol_index{0}; symbol_index < num_symbols; symbol_index++) {
        uint32_t symbol = 0, length = 0; // , code = 0; // not necessary, see the comment below
        br.read(symbol, 8);
        br.read(length, 5);
        // br.read(code, length);  // canonical huffman codes are reconstructed on the fly, not read from the table
        ...
        const std::string huffman_code = uint32_to_binary_string(current_code, length);
